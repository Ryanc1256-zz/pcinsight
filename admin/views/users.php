	...
	class Page extends view{
		...
		function getAllUsers(){
			...
			for ($i = 0; $i < count($users); $i++){
				$rs = $users[$i];
				$registered = $rs['registered'] == true ? 'true' : 'false';
				$html .= '<tr><td>' . $rs['username'] . '</td><td>'. $rs['email'] .'</td><td>'. $rs['usertype'] .'</td><td>'.  $registered . ' </td><td>' . $rs['registeredDate'] . '</td><td><a href="index.php?p=5&usr='.$rs[id].'" class="btn red"> Edit </a></td></tr>'."\n";
			}
			return $html;
