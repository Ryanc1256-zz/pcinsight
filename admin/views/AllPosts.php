	...
	class Page extends view{
		...
		function posts(){
			...
			for ($i = 0; $i < $amount; $i++){
				...
				$conf = $result['postConfirmed'] == true ? 'true' : 'false'; 
				$list .= '<tr class="post"><td>' . $writer['Username'] . '</td><td>' . $result['postTitle'] .'</td><td>' . $result['postDate'] . '</td><td>'.$conf.'</td><td><a href="index.php?p=6&id='.$result['postID'].'" class="btn red">Edit</a></tr>'."\n";
			}
			return $list;
		}
	}		
